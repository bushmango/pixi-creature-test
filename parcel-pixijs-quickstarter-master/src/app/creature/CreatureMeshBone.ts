...
dualQuat.prototype.createFromData = function (q0, t) {
  ...
  this.imaginary = quat.create();
  this.imaginary[Q_W] =
    -0.5 * (t[Q_X] * q0[Q_X] + t[Q_Y] * q0[Q_Y] + t[Q_Z] * q0[Q_Z]);
  this.imaginary[Q_X] =
    0.5 * (t[Q_X] * q0[Q_W] + t[Q_Y] * q0[Q_Z] - t[Q_Z] * q0[Q_Y]);
  this.imaginary[Q_Y] =
    0.5 * (-t[Q_X] * q0[Q_Z] + t[Q_Y] * q0[Q_W] + t[Q_Z] * q0[Q_X]);
  this.imaginary[Q_Z] =
    0.5 * (t[Q_X] * q0[Q_Y] - t[Q_Y] * q0[Q_X] + t[Q_Z] * q0[Q_W]);
};
...
MeshBone.prototype.setLocalRestStartPt = function (world_pt_in) {
  ...
  this.local_rest_start_pt = vec3.transformMat4(
    this.local_rest_start_pt,
    world_pt_in,
    this.rest_parent_inv_mat
  );
  this.calcRestData();
};
...
MeshBone.prototype.setLocalRestEndPt = function (world_pt_in) {
  ...
  this.local_rest_end_pt = vec3.transformMat4(
    this.local_rest_end_pt,
    world_pt_in,
    this.rest_parent_inv_mat
  );
  this.calcRestData();
...
MeshBone.prototype.fixDQs = function (ref_dq) {
  ...
  if (quat.dot(this.world_dq.real, ref_dq.real) < 0) {
    ...
    this.world_dq.imaginary = quat.scale(
      this.world_dq.imaginary,
      this.world_dq.imaginary,
      -1
    );
  }
...
MeshBone.prototype.computeRestParentTransforms = function () {
  var cur_tangent = vec3.fromValues(
    this.local_rest_dir[Q_X],
    this.local_rest_dir[Q_Y],
    0
  );
  var cur_binormal = vec3.fromValues(
    this.local_binormal_dir[Q_X],
    this.local_binormal_dir[Q_Y],
    this.local_binormal_dir[Q_Z]
  );
  var cur_normal = vec3.fromValues(
    this.local_rest_normal_dir[Q_X],
    this.local_rest_normal_dir[Q_Y],
    0
  );
  ...
  this.rest_world_mat = mat4.create();
  this.rest_world_mat = mat4.multiply(
    this.rest_world_mat,
    this.rest_parent_mat,
    cur_final
  );

  this.rest_world_inv_mat = mat4.clone(this.rest_world_mat);
  this.rest_world_inv_mat = mat4.invert(
    this.rest_world_inv_mat,
    this.rest_world_inv_mat
  );
  ...
  world_rest_dir = vec3.subtract(
    world_rest_dir,
    world_rest_dir,
    this.getWorldRestStartPt()
  );
  ...
  this.bind_world_inv_mat = mat4.invert(
    this.bind_world_inv_mat,
    this.bind_world_inv_mat
  );
...
MeshBone.prototype.computeParentTransforms = function () {
  ...
  translate_parent = mat4.translate(
    translate_parent,
    translate_parent,
    this.getWorldEndPt()
  );
...
MeshBone.prototype.computeWorldDeltaTransforms = function () {
  ...
  this.world_delta_mat = mat4.multiply(
    this.world_delta_mat,
    cur_translate,
    cur_rotate
  );
  this.world_delta_mat = mat4.multiply(
...
MeshRenderRegion.prototype.poseFinalPts = function (
  output_pts,
  output_start_index,
  bones_map
) {
  ...
  for (var i = 0, l = this.getNumPts(); i < l; i++) {
    var cur_rest_pt = vec3.set(
      tmp1,
      this.store_rest_pts[0 + read_pt_index],
...
MeshRenderRegion.prototype.setUseLocalDisplacements = function (flag_in) {
  ...
  if (
    this.local_displacements.length != this.getNumPts() &&
...
MeshRenderRegion.prototype.setUsePostDisplacements = function (flag_in) {
  ...
  if (
    this.post_displacements.length != this.getNumPts() &&
...
MeshRenderRegion.prototype.initUvWarp = function () {
  ...
  for (var i = 0; i < this.getNumPts(); i++) {
    ...
    this.uv_warp_ref_uvs[i] = vec2.fromValues(
      this.store_uvs[cur_uvs_index],
...
MeshRenderBoneComposition.prototype.initRegionsMap = function () {
  ...
  for (var i = 0; i < this.regions.length; i++) {
    let cur_key = this.regions[i].getName();
...
MeshRenderBoneComposition.prototype.updateAllTransforms = function (
  update_parent_xf
) {
  if (update_parent_xf) {
...
MeshDisplacementCache.prototype.setLocalDisplacements = function (
  displacements_in
) {
  this.local_displacements = displacements_in;
};

MeshDisplacementCache.prototype.setPostDisplacements = function (
  displacements_in
) {
...
MeshBoneCacheManager.prototype.retrieveValuesAtTime = function (
  time_in,
  bone_map
) {
  ...
  if (
    this.bone_cache_data_ready[base_time] == false ||
  ...
  for (var i = 0, l = base_cache.length; i < l; i++) {
    ...
    var final_world_start_pt = Utils.vecInterp(
      base_data.getWorldStartPt(),
      end_data.getWorldStartPt(),
      ratio
    );

    var final_world_end_pt = Utils.vecInterp(
      base_data.getWorldEndPt(),
      end_data.getWorldEndPt(),
      ratio
    );
...
MeshDisplacementCacheManager.prototype.init = function (
  start_time_in,
  end_time_in
) {
  this.start_time = start_time_in;
...
MeshDisplacementCacheManager.prototype.retrieveValuesAtTime = function (
  time_in,
  regions_map
) {
  var base_time = this.getIndexByTime(Math.floor(time_in));
  ...
  if (
    this.displacement_cache_data_ready[base_time] == false ||
  ...
  for (var i = 0; i < base_cache.length; i++) {
    ...
    if (set_region.getUseLocalDisplacements()) {
      ...
      if (
        base_data.getLocalDisplacements().length == displacements.length &&
        end_data.getLocalDisplacements().length == displacements.length
      ) {
        for (var j = 0; j < displacements.length; j++) {
          var interp_val = Utils.vec2Interp(
            base_data.getLocalDisplacements()[j],
    ...
    if (set_region.getUsePostDisplacements()) {
      ...
      if (
        base_data.getPostDisplacements().length == displacements.length &&
        end_data.getPostDisplacements().length == displacements.length
      ) {
        for (var j = 0; j < displacements.length; j++) {
          var interp_val = Utils.vec2Interp(
            base_data.getPostDisplacements()[j],
...
MeshUVWarpCacheManager.prototype.retrieveValuesAtTime = function (
  time_in,
  regions_map
) {
  var base_time = this.getIndexByTime(Math.floor(time_in));
  ...
  if (
    this.uv_cache_data_ready[base_time] == false ||
...
MeshOpacityCacheManager.prototype.retrieveValuesAtTime = function (
  time_in,
  regions_map
) {
  var base_time = this.getIndexByTime(Math.floor(time_in));
  ...
  if (
    this.opacity_cache_data_ready[base_time] == false ||
...
MeshOpacityCacheManager.prototype.allReady = function () {
  if (this.is_ready) {
    ...
    for (var i = 0; i < this.opacity_cache_data_ready.length; i++) {
      if (this.opacity_cache_data_ready[i]) {
        ready_cnt++;
      }
    }
...
CreatureModuleUtils.CreateBones = function (json_obj, key) {
  ...
  for (var cur_name in base_obj) {
    ...
    var cur_parent_mat = CreatureModuleUtils.ReadMatrixJSON(
      cur_node,
      "restParentMat"
    );

    var cur_local_rest_start_pt = CreatureModuleUtils.ReadVector3JSON(
      cur_node,
      "localRestStartPt"
    );
    var cur_local_rest_end_pt = CreatureModuleUtils.ReadVector3JSON(
      cur_node,
      "localRestEndPt"
    );
    var cur_children_ids = CreatureModuleUtils.ReadIntArrayJSON(
      cur_node,
      "children"
    );

    var new_bone = new MeshBone(
      cur_name,
      vec3.create(),
      vec3.create(),
      cur_parent_mat
...
CreatureModuleUtils.CreateBonesFlat = function (skelIn) {
  ...
  for (var i = 0; i < skelIn.bonesLength(); i++) {
    ...
    var cur_id = cur_node.id(); //GetJSONNodeFromKey(*cur_node, "id")->value.toNumber();
    var cur_parent_mat = CreatureModuleUtils.ReadMatrixFlat(
      cur_node.restParentMatArray()
    );

    var cur_local_rest_start_pt = CreatureModuleUtils.ReadFloatArray3DFlat(
      cur_node.localRestStartPtArray()
    );
    var cur_local_rest_end_pt = CreatureModuleUtils.ReadFloatArray3DFlat(
      cur_node.localRestEndPtArray()
    );
    var cur_children_ids = CreatureModuleUtils.getIntArray(
      cur_node.childrenArray()
    );
...
CreatureModuleUtils.CreateRegions = function (
  json_obj,
  key,
  indices_in,
  rest_pts_in,
  ...
  for (var cur_name in base_obj) {
    ...
    var new_region = new MeshRenderRegion(
      indices_in,
      rest_pts_in,
      uvs_in,
      cur_start_pt_index,
    ...
    // Read in weights
    var weight_map = new_region.normal_weight_map;
...
CreatureModuleUtils.CreateRegionsFlat = function (
  meshIn,
  indices_in,
  rest_pts_in,
  uvs_in
...
CreatureModuleUtils.FillBoneGapCache = function (
  prev_time,
  cur_time,
  set_index,
  cache_manager,
    ...
    for (var j = 1; j < gap_diff; j++) {
      ...
      for (var k = 0; k < cache_list.length; k++) {
        ...
        gap_cache_data.setWorldStartPt(
          Utils.vecInterp(
            prev_data.getWorldStartPt(),
            cur_data.getWorldStartPt(),
            gap_fraction
          )
        );
        gap_cache_data.setWorldEndPt(
          Utils.vecInterp(
            prev_data.getWorldEndPt(),
            cur_data.getWorldEndPt(),
            gap_fraction
          )
        );
...
CreatureModuleUtils.FillBoneCache = function (
  json_obj,
  key,
  start_time,
  end_time,
  ...
  for (var cur_time in base_obj) {
    ...
    for (var cur_name in cur_node) {
      ...
      var cur_start_pt = CreatureModuleUtils.ReadVector3JSON(
        bone_node,
        "start_pt"
      ); //ReadJSONVec4_2(*bone_node, "start_pt");
    ...
    CreatureModuleUtils.FillBoneGapCache(
      prev_time,
      cur_time,
      set_index,
      cache_manager,
...
CreatureModuleUtils.FillBoneCacheFlat = function (
  animBonesList,
  start_time,
  end_time,
  cache_manager
  ...
  for (var i = 0; i < animBonesList.timeSamplesLength(); i++) {
    ...
    for (var j = 0; j < cur_node.bonesLength(); j++) {
      ...
      var cur_start_pt = CreatureModuleUtils.ReadFloatArray3DFlat(
        bone_node.startPtArray()
      ); //ReadJSONVec4_2(*bone_node, "start_pt");
      var cur_end_pt = CreatureModuleUtils.ReadFloatArray3DFlat(
        bone_node.endPtArray()
      ); //ReadJSONVec4_2(*bone_node, "end_pt");
...
CreatureModuleUtils.FillDeformationGapCache = function (
  prev_time,
  cur_time,
  set_index,
  cache_manager,
      ...
      for (var k = 0; k < cache_list.length; k++) {
        ...
        if (cur_data.getLocalDisplacements().length > 0) {
          gap_cache_data.setLocalDisplacements(
            Utils.ptsInterp(
              prev_data.getLocalDisplacements(),
              cur_data.getLocalDisplacements(),
              gap_fraction
            )
          );
        } else {
          gap_cache_data.setPostDisplacements(
            Utils.ptsInterp(
              prev_data.getPostDisplacements(),
              cur_data.getPostDisplacements(),
              gap_fraction
            )
          );
        }
...
CreatureModuleUtils.FillDeformationCache = function (
  json_obj,
  key,
  start_time,
  end_time,
  ...
  for (var cur_time in base_obj) {
    ...
    for (var cur_name in cur_node) {
      ...
      var use_local_displacement = CreatureModuleUtils.ReadBoolJSON(
        mesh_node,
        "use_local_displacements"
      ); //GetJSONNodeFromKey(*mesh_node, "use_local_displacements")->value.toBool();
      var use_post_displacement = CreatureModuleUtils.ReadBoolJSON(
        mesh_node,
        "use_post_displacements"
      ); //GetJSONNodeFromKey(*mesh_node, "use_post_displacements")->value.toBool();

      if (use_local_displacement == true) {
        var read_pts = CreatureModuleUtils.ReadPointsArray2DJSON(
          mesh_node,
          "local_displacements"
        ); //ReadJSONPoints2DVector(*mesh_node, "local_displacements");
    ...
    CreatureModuleUtils.FillDeformationGapCache(
      prev_time,
      cur_time,
      set_index,
      cache_manager,
...
CreatureModuleUtils.FillDeformationCacheFlat = function (
  animMeshList,
  start_time,
  end_time,
  cache_manager
  ...
  for (var i = 0; i < animMeshList.timeSamplesLength(); i++) {
    ...
    for (var j = 0; j < cur_node.meshesLength(); j++) {
      ...
      if (use_local_displacement == true) {
        var read_pts = CreatureModuleUtils.ReadPointsArray2DFlat(
          mesh_node.localDisplacementsArray()
        ); //ReadJSONPoints2DVector(*mesh_node, "local_displacements");
        cache_data.setLocalDisplacements(read_pts);
      }
      ...
      if (use_post_displacement == true) {
        ...
        var read_pts = CreatureModuleUtils.ReadPointsArray2DFlat(
          mesh_node.postDisplacementsArray()
        ); //ReadJSONPoints2DVector(*mesh_node, "post_displacements");
        cache_data.setPostDisplacements(read_pts);
      }
...
CreatureModuleUtils.FillUVSwapCache = function (
  json_obj,
  key,
  start_time,
  end_time,
  ...
  for (var cur_time in base_obj) {
    ...
    for (var cur_name in cur_node) {
      ...
      if (use_uv == true) {
        var local_offset = CreatureModuleUtils.ReadVector2JSON(
          uv_node,
          "local_offset"
        ); //ReadJSONVec2(*uv_node, "local_offset");
        var global_offset = CreatureModuleUtils.ReadVector2JSON(
          uv_node,
          "global_offset"
        ); //ReadJSONVec2(*uv_node, "global_offset");
...
CreatureModuleUtils.FillUVSwapCacheFlat = function (
  animUVList,
  start_time,
  end_time,
  cache_manager
  ...
  for (var i = 0; i < animUVList.timeSamplesLength(); i++) {
    ...
    for (var j = 0; j < cur_node.uvSwapsLength(); j++) {
      ...
      cache_data.setEnabled(use_uv);
      if (use_uv == true) {
        var local_offset = CreatureModuleUtils.ReadVector2Flat(
          uv_node.localOffsetArray()
        ); //ReadJSONVec2(*uv_node, "local_offset");
        var global_offset = CreatureModuleUtils.ReadVector2Flat(
          uv_node.globalOffsetArray()
        ); //ReadJSONVec2(*uv_node, "global_offset");
...
CreatureModuleUtils.FillOpacityGapCache = function (
  prev_time,
  cur_time,
  set_index,
  cache_manager,
  ...
  if (gap_diff > 1) {
    ...
    for (var j = 1; j < gap_diff; j++) {
      ...
      for (var k = 0; k < cache_list.length; k++) {
        ...
        gap_cache_data.setOpacity(
          Utils.scalarInterp(
            prev_data.getOpacity(),
            cur_data.getOpacity(),
            gap_fraction
          )
        );
...
CreatureModuleUtils.FillOpacityCache = function (
  json_obj,
  key,
  start_time,
  end_time,
  ...
  for (var cur_time in base_obj) {
    ...
    CreatureModuleUtils.FillOpacityGapCache(
      prev_time,
      cur_time,
      set_index,
      cache_manager,
...
CreatureModuleUtils.FillOpacityCacheFlat = function (
  animOpacityList,
  start_time,
  end_time,
  cache_manager
...
CreatureModuleUtils.FillSwapUVPacketMap = function (json_obj) {
  ...
  for (var cur_key in json_obj) {
    ...
    for (var i = 0; i < cur_node.length; i++) {
      ...
      var new_packet = new CreatureUVSwapPacket(
        local_offset,
        global_offset,
        scale,
        tag
...
CreatureModuleUtils.FillSwapUVPacketMapFlat = function (uvSwapItemFlatHolder) {
  ...
  for (var i = 0; i < uvSwapItemFlatHolder.meshesLength(); i++) {
    ...
    for (var j = 0; j < cur_node.itemsLength(); j++) {
      ...
      var local_offset = CreatureModuleUtils.ReadVector2Flat(
        packet_node.localOffsetArray()
      );
      var global_offset = CreatureModuleUtils.ReadVector2Flat(
        packet_node.globalOffsetArray()
      );
...
// Creature
export function Creature(load_data, use_flat_data) {
...
Creature.prototype.ShouldSkinSwap = function () {
  return (
    this.creature_meta_data &&
    this.skin_swap_active &&
    this.final_skin_swap_indices
  );
};
...
Creature.prototype.ComputeBoundaryMinMax = function () {
  ...
  var minPt = vec2.fromValues(
    this.render_pts[firstIdx + 0],
    this.render_pts[firstIdx + 1]
  );
...
Creature.prototype.LoadFromData = function (load_data) {
  ...
  this.global_pts = CreatureModuleUtils.ReadFloatArray3DJSON(
    json_mesh,
    "points"
  );
  this.total_num_pts = this.global_pts.length / 3;

  this.global_indices = CreatureModuleUtils.ReadIntArrayJSON(
    json_mesh,
    "indices"
  );
  ...
  // Load regions
  var regions = CreatureModuleUtils.CreateRegions(
    json_mesh,
    "regions",
    this.global_indices,
    this.global_pts,
  ...
  if ("uv_swap_items" in load_data) {
    ...
    this.uv_swap_packets = CreatureModuleUtils.FillSwapUVPacketMap(
      json_uv_swap_base
    );
  }
  ...
  if ("anchor_points_items" in load_data) {
    ...
    this.anchor_point_map = CreatureModuleUtils.FillAnchorPointMap(
      anchor_point_base
    );
  }
};
...
Creature.prototype.LoadFromDataFlat = function (flatRoot) {
  ...
  this.global_pts = CreatureModuleUtils.ReadFloatArray3DFlat(
    flat_mesh.pointsArray()
  );
  this.total_num_pts = this.global_pts.length / 3;

  this.global_indices = CreatureModuleUtils.getIntArray(
    flat_mesh.indicesArray()
  );
  this.total_num_indices = this.global_indices.length;
  ...
  // Load regions
  var regions = CreatureModuleUtils.CreateRegionsFlat(
    flat_mesh,
    this.global_indices,
    this.global_pts,
    this.global_uvs
  ...
  this.uv_swap_packets = CreatureModuleUtils.FillSwapUVPacketMapFlat(
    flat_uv_swap_item_holder
  );

  // Load Anchor Points
  var flat_anchor_holder = flatRoot.dataAnchorPoints();
  this.anchor_point_map = CreatureModuleUtils.FillAnchorPointMapFlat(
    flat_anchor_holder
  );
};

// CreatureAnimation
export function CreatureAnimation(load_data, name_in, use_flat_data?) {
...
CreatureAnimation.prototype.LoadFromData = function (name_in, load_data) {
  ...
  var start_end_times = CreatureModuleUtils.GetStartEndTimes(
    json_clip,
    "bones"
  );
  ...
  // bone animation
  CreatureModuleUtils.FillBoneCache(
    json_clip,
    "bones",
    this.start_time,
    this.end_time,
  ...
  CreatureModuleUtils.FillDeformationCache(
    json_clip,
    "meshes",
    this.start_time,
    this.end_time,
  ...
  CreatureModuleUtils.FillUVSwapCache(
    json_clip,
    "uv_swaps",
    this.start_time,
    this.end_time,
  ...
  // opacity animation
  CreatureModuleUtils.FillOpacityCache(
    json_clip,
    "mesh_opacities",
    this.start_time,
    this.end_time,
...
CreatureAnimation.prototype.LoadFromDataFlat = function (name_in, animFlat) {
  ...
  var start_end_times = CreatureModuleUtils.GetStartEndTimesFlat(
    flat_clip.bones()
  );
  this.start_time = start_end_times.first;
  this.end_time = start_end_times.second;
  ...
  // bone animation
  CreatureModuleUtils.FillBoneCacheFlat(
    flat_clip.bones(),
    this.start_time,
    this.end_time,
    this.bones_cache
  ...
  // mesh deformation animation
  CreatureModuleUtils.FillDeformationCacheFlat(
    flat_clip.meshes(),
    this.start_time,
    this.end_time,
    this.displacement_cache
  ...
  // uv swapping animation
  CreatureModuleUtils.FillUVSwapCacheFlat(
    flat_clip.uvSwaps(),
    this.start_time,
    this.end_time,
    this.uv_warp_cache
  ...
  // opacity animation
  CreatureModuleUtils.FillOpacityCacheFlat(
    flat_clip.meshOpacities(),
    this.start_time,
    this.end_time,
    this.opacity_cache
  );
...
CreatureAnimation.prototype.poseFromCachePts = function (
  time_in,
  target_pts,
  num_pts
) {
  ...
  for (var i = 0; i < num_pts; i++) {
    set_pt[set_idx + 0] =
      (1.0 - cur_ratio) * floor_pts[floor_idx + 0] +
      cur_ratio * ceil_pts[ceil_idx + 0];
    set_pt[set_idx + 1] =
      (1.0 - cur_ratio) * floor_pts[floor_idx + 1] +
      cur_ratio * ceil_pts[ceil_idx + 1];
    set_pt[set_idx + 2] =
      (1.0 - cur_ratio) * floor_pts[floor_idx + 2] +
      cur_ratio * ceil_pts[ceil_idx + 2];
...
CreatureGameController.prototype.BuildFrameCallbacks = function (
  assignCallbackFn
) {
  ...
  for (var cur_key in this.meta_asset.anim_events_map) {
    ...
    for (var cur_evt_key in cur_val) {
      ...
      new_callback.callback = assignCallbackFn(new_callback.animClipName, name);
...
CreatureGameController.prototype.AddFrameCallback = function (
  animClipName,
  name,
  frame,
  callbackFb
...
CreatureGameController.prototype.ProcessCallbacks = function (
  creature_manager
) {
...
CreatureMetaData.prototype.buildSkinSwapIndices = function (
  swap_name,
  bone_composition
) {
...
// CreatureManager
export function CreatureManager(target_creature_in) {
...
CreatureManager.prototype.GetAnimationNames = function () {
  var ret_names = [];
  for (var cur_name in this.animations) {
...
CreatureManager.prototype.SetActiveAnimationName = function (
  name_in,
...
CreatureManager.prototype.FillSinglePointCacheFrame = function (
  animation_name_in,
  time_in
) {
  var store_run_time = this.getRunTime();
...
CreatureManager.prototype.SetCustomTimeRange = function (
  start_time_in,
  end_time_in
) {
...
CreatureManager.prototype.correctRunTime = function (time_in, animation_name) {
  let ret_time = time_in;
  let cur_animation = this.animations[animation_name];
  let anim_start_time = cur_animation.start_time;
  let anim_end_time = cur_animation.end_time;
...
CreatureManager.prototype.checkAnimationBlendValid = function () {
  for (var i = 0; i < 2; i++) {
    let cur_animation_name = this.active_blend_animation_names[i];
    if (
      !(cur_animation_name in this.animations) ||
...
CreatureManager.prototype.UpdateRegionsSwitches = function (animation_name_in) {
  ...
  var uv_swap_table = uv_warp_cache_manager.uv_cache_table[0];

  var render_composition = this.target_creature.render_composition;
  ...
  for (var i = 0; i < all_regions.length; i++) {
    ...
    var use_local_displacements = !(
      displacement_table[index].getLocalDisplacements().length == 0
    );
    var use_post_displacements = !(
      displacement_table[index].getPostDisplacements().length == 0
    );
...
CreatureManager.prototype.RunCreature = function () {
  if (this.do_blending) {
    for (var i = 0; i < 2; i++) {
      ...
      let cur_animation_run_time = this.active_blend_run_times[
        cur_animation_name
      ];

      if (cur_animation.cache_pts.length > 0) {
        this.UpdateRegionsSwitches(cur_animation_name);
        cur_animation.poseFromCachePts(
          cur_animation_run_time,
          this.blend_render_pts[i],
          this.target_creature.total_num_pts
        );
      } else {
        this.UpdateRegionsSwitches(cur_animation_name);
        this.PoseCreature(
          this.active_blend_animation_names[i],
          this.blend_render_pts[i],
          cur_animation_run_time
        );
      }
...
CreatureManager.prototype.AlterBonesByAnchor = function (
  bones_map,
  animation_name_in
) {
  ...
  for (var cur_bone_key in bones_map) {
    ...
    start_pt = vec3.subtract(
      start_pt,
      start_pt,
      vec3.fromValues(anchor_point[0], anchor_point[1], 0)
    );
    end_pt = vec3.subtract(
      end_pt,
      end_pt,
      vec3.fromValues(anchor_point[0], anchor_point[1], 0)
    );
...
CreatureManager.prototype.AutoBlendTo = function (
  animation_name_in,
  blend_delta
) {
  ...
  this.SetBlendingAnimations(
    this.auto_blend_names[0],
    this.auto_blend_names[1]
  );
};

CreatureManager.prototype.ResetBlendTime = function (name_in) {
  let cur_animation = this.animations[name_in];
...
CreatureManager.prototype.PoseCreature = function (
  animation_name_in,
  target_pts,
  input_run_time
) {
  ...
  var bones_map = render_composition.getBonesMap();
  var regions_map = render_composition.getRegionsMap();
  ...
  displacement_cache_manager.retrieveValuesAtTime(input_run_time, regions_map);
  uv_warp_cache_manager.retrieveValuesAtTime(input_run_time, regions_map);
  opacity_cache_manager.retrieveValuesAtTime(input_run_time, regions_map);

  // Do posing, decide if we are blending or not
  var cur_regions = render_composition.getRegions();
  ...
  for (var j = 0, l = cur_regions.length; j < l; j++) {
    ...
    cur_region.poseFinalPts(target_pts, cur_pt_index * 3, cur_bones);
