...
export function CreatureRenderer(manager_in, texture_in) {
...
// constructor
CreatureRenderer.prototype = Object.create(
...
CreatureRenderer.prototype._renderCreature = function (renderSession) {
  ...
  // set uniforms
  gl.uniformMatrix3fv(
    shader.translationMatrix,
    false,
    this.worldTransform.toArray(true)
  );
  ...
  if (!this.dirty) {
    ...
    if (this.texture.baseTexture._dirty[gl.id]) {
      ...
      gl.bindTexture(
        gl.TEXTURE_2D,
        this.texture.baseTexture._glTextures[gl.id]
      );
    }
...
CreatureRenderer.prototype.UpdateCreatureBounds = function () {
  ...
  this.creatureBoundsMin.set(
    target_creature.boundary_min[0],
    -target_creature.boundary_min[1]
  );
  this.creatureBoundsMax.set(
    target_creature.boundary_max[0],
...
CreatureRenderer.prototype.EnableSkinSwap = function (swap_name_in, active) {
  ...
  this.indices = new Uint16Array(
    target_creature.final_skin_swap_indices.length
  );
  for (var i = 0; i < this.indices.length; i++) {
...
CreatureRenderer.prototype.GetPixelScaling = function (desired_x, desired_y) {
  ...
  var mesh_size_x =
    target_creature.boundary_max[0] - target_creature.boundary_min[0];
  var mesh_size_y =
    target_creature.boundary_max[1] - target_creature.boundary_min[1];
...
CreatureRenderer.prototype.UpdateRenderData = function (inputVerts, inputUVs) {
  ...
  var render_composition = target_creature.render_composition;
